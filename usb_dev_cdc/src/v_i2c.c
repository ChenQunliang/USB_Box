...
int32_t I2C_Master_Transmit(uint16_t u16DevAddr, uint8_t const au8Data[], uint32_t u32Size, uint32_t u32Timeout)
{
    ...
    if (LL_OK == i32Ret)
    {
        ...
        if (LL_OK == i32Ret)
        {
            i32Ret = I2C_TransData(V_I2C_UNIT, au8Data, u32Size, u32Timeout);
        }
    }

    (void)I2C_Stop(V_I2C_UNIT, u32Timeout);
...
int32_t ADV_7280_Send_Buff(uint8_t const au8Data[], uint32_t u32Size, uint32_t u32Timeout)
{
    ...
    while ((i32Count) < u32Size)
    {
        ...
        if (LL_OK == i32Ret)
        {
            i32Ret = I2C_TransAddr(V_I2C_UNIT, (uint16_t)au8Data[i32Count * 3] >> 1, I2C_DIR_TX, u32Timeout);
            DDL_DelayUS(10);
            if (LL_OK == i32Ret)
            {
                i32Ret = I2C_TransData(V_I2C_UNIT, &au8Data[i32Count * 3 + 1], 2, u32Timeout);
            }
        }
        (void)I2C_Stop(V_I2C_UNIT, u32Timeout);
        I2C_Cmd(V_I2C_UNIT, DISABLE);
...
int32_t I2C_Master_Receive(uint16_t u16DevAddr, uint8_t au8Data[], uint32_t u32Size, uint32_t u32Timeout)
{
    ...
    if (LL_OK == i32Ret)
    {
        ...
        if (LL_OK == i32Ret)
        {
            i32Ret = I2C_MasterReceiveDataAndStop(V_I2C_UNIT, au8Data, u32Size, u32Timeout);
        }
...
int32_t Chip_Receive(uint16_t u16DevAddr, uint8_t au8Data_tx[], uint8_t au8Data_rx[], uint32_t u32Size, uint32_t u32Timeout)
{
    ...
    if (LL_OK == i32Ret)
    {
        ...
        if (LL_OK == i32Ret)
        {
            i32Ret = I2C_TransData(V_I2C_UNIT, au8Data_tx, u32Size, u32Timeout);
        }
    }
    DDL_DelayUS(10);
    // Receive
    i32Ret = I2C_Restart(V_I2C_UNIT, u32Timeout);
    ...
    if (LL_OK == i32Ret)
    {
        ...
        if (LL_OK == i32Ret)
        {
            i32Ret = I2C_MasterReceiveDataAndStop(V_I2C_UNIT, au8Data_rx, u32Size, u32Timeout);
        }
...
int32_t Master_Initialize(void)
{
    ...
    I2C_DeInit(V_I2C_UNIT);

    (void)I2C_StructInit(&stcI2cInit);
    stcI2cInit.u32ClockDiv = V_I2C_CLK_DIVx;
    stcI2cInit.u32Baudrate = V_I2C_BAUDRATE;
...
void V_I2C_Init(void)
{
    ...
    FCG_Fcg1PeriphClockCmd(V_I2C_FCG_USE, ENABLE);
